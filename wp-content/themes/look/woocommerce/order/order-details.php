...
<div class="text-heading">
	<h2 class="module-title"><span><?php _e( 'Order Details', 'look' ); ?></span></h2>
</div>
<table class="shop_table order_details">
	<thead>
		<tr>
			<th class="product-name"><?php _e( 'Product', 'look' ); ?></th>
			<th class="product-total"><?php _e( 'Total', 'look' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
						...
						<td class="product-name">
							...
							if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
								...
								foreach ( $download_files as $download_id => $file ) {
									...
									$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'look' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
								}
	...
	<tfoot>
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'look' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'look' ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
		<tr>
			<th scope="row"><?php _e( 'Refunded:', 'look' ); ?></th>
	...
	<tr>
		<th scope="row"><?php _e( 'Note:', 'look' ); ?></th>
...
<div class="text-heading">
	<h2 class="module-title"><span><?php _e( 'Customer details', 'look' ); ?></span></h2>
</div>
<table class="shop_table shop_table_responsive customer_details">
	<?php
	if ( $order->billing_email ) {
		echo '<tr><th>' . __( 'Email:', 'look' ) . '</th><td data-title="' . __( 'Email', 'look' ) . '">' . $order->billing_email . '</td></tr>';
	}

	if ( $order->billing_phone ) {
		echo '<tr><th>' . __( 'Telephone:', 'look' ) . '</th><td data-title="' . __( 'Telephone', 'look' ) . '">' . $order->billing_phone . '</td></tr>';
	}
